...
namespace APIMongoCrud.Controllers
{
    ...
    {
        private IRepository<Pago> db = new BS.Pagos();
        ...
        //Metodo para crear
        [HttpPost]
        public async Task<ActionResult> Create([FromBody] Pago Pago)
        {
            if (Pago == null)
                return BadRequest();
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> Update([FromBody] Pago Pago, string id)
        {
            if (Pago == null)
                return BadRequest();
